...
export function Home() {
  ...
  async function nextVerse() {
    ...
    try {
      ...
      if (!response) {
        setApiResult(null);
        showMessage.info('Pulando capitulo');
      }
  ...
  function salveVerse() {
    ...
    const currentVerse: currentVerseModel = {
      book: bookName,
      chapter: bookChapter,
      verse: bookVerse,
      verseText: apiResult,
    ...
    const history = JSON.parse(localStorage.getItem('versesHistory') || '[]');

    history.push(currentVerse);

    localStorage.setItem('versesHistory', JSON.stringify(history));

    showMessage.success('Versículo salvo nos favoritos!');
  }
  ...
  return (
    <div className="p-5 bg-[linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.7)),url('/img/backGroundBible.jpg')] bg-cover bg-center w-screen h-screen">
      <Link
        to='/favoritePage'
        className='text-white hover:cursor-pointer hover:text-gray-500 transition-all'
      >
        <StarIcon />
      </Link>
      <form
